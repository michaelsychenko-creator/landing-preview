  ...
  return (
    <section className="bg-white py-16 sm:py-20 lg:py-24">
      <div className="reveal-on-scroll mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          {hotels.map((hotel) => (
            <motion.li key={hotel.name} variants={itemVariants} className="flex flex-col">
              ...
              <div className="mt-5 flex flex-1 flex-col">
                ...
                <p className="mt-2 text-sm leading-relaxed text-navy-muted">{hotel.description}</p>
                <p className="mt-3 text-xs tracking-wide text-navy/60">{hotel.tags}</p>
                <Separator className="my-4 bg-navy/10" />
                <div className="mt-auto flex items-end justify-between gap-3">
                  <p className="text-sm text-navy-muted">
                    from <span className="text-2xl font-semibold text-navy">{hotel.price}</span>
